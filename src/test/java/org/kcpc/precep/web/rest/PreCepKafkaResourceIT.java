...
import org.kcpc.precep.config.KafkaProperties;
...
class PreCepKafkaResourceIT {
    ...
    void setup() {
        ...
        PreCepKafkaResource kafkaResource = new PreCepKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/pre-cep-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/pre-cep-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
